...
class FFP_REST {
    ...
    public function routes() {
        ...
        register_rest_route('ffp/v1','/orders/(?P<id>\d+)/status',[
            'methods'=>'POST',
            'permission_callback'=>function(){ return current_user_can('ffp_update_orders') || current_user_can('ffp_driver'); },
            'callback'=>[$this,'update_status']
        ]);

        register_rest_route('ffp/v1','/orders/(?P<id>\d+)/claim',[
            'methods'=>'POST',
            'permission_callback'=>function(){ return current_user_can('ffp_driver'); },
    ...
    public function update_status($req) {
        ...
        if (!in_array($status, $allowed, true)) return new WP_Error('ffp_bad_status','Invalid status', ['status'=>400]);
